...
export const uploadLectureVideo = async (req, res) => {
  ...
  try {
    ...
    const oldDuration = Number(lecture.duration || 0);
    const durationDiff = newDuration - oldDuration;

    section.totalDuration = Number(section.totalDuration || 0) + durationDiff;
    course.totalDuration = Number(course.totalDuration || 0) + durationDiff;
...
export const updateLectureVideo = async (req, res) => {
  ...
  try {
    ...
    const { videoUrl, videoProvider = "youtube", duration } = req.body;

    // Scenario 1: Update with YouTube URL
    if (videoUrl && videoProvider === "youtube") {
      if (isBlank(videoUrl)) {
        return AppError(res, "Video URL cannot be blank", 400, req.file);
      }
      ...
      if (duration !== undefined) {
        ...
        section.totalDuration = Number(section.totalDuration) || 0;
        course.totalDuration = Number(course.totalDuration) || 0;
    ...
    // Scenario 2: Upload new video file to S3
    if (!req.file) {
    ...
    // Validate duration
    const newDuration = Number(duration || req.body.duration);
...
export const uploadLectureAttachment = async (req, res) => {
  ...
  try {
    ...
    if (!req.file) {
      return AppError(res, "No attachment file uploaded", 400, req.file);
    }
    if (req.file) {
      try {
        const { publicId, secureUrl } = await uploadAttachment(req.file.path);
...
export const deleteLectureAttachment = async (req, res) => {
  try {
    const { lectureId } = req.params;
    const userId = req.user?.id;
    if (!mongoose.Types.ObjectId.isValid(lectureId)) {
    ...
    const section = await Section.findById(lecture.section).populate("course");
    const course = section.course;
    if (course.instructor.toString() !== instructor._id.toString()) {
      return AppError(res, "Unauthorized access", 403);
    }
    if (lecture.attachment && lecture.attachment.publicId) {
      const publicId = lecture.attachment.publicId;
      await deleteAttachment(publicId);
      lecture.attachment = null;
      await lecture.save();
    }
    return ApiResponse(res, {
      statusCode: 200,
      message: "Lecture attachment deleted successfully",
    });
  } catch (error) {
    return AppError(res, "Failed to delete lecture attachment", 500);
  }
};
